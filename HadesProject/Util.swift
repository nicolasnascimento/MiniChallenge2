...
class Util: NSObject {
    ...
    static func resizeSprite(_ sprite: SKSpriteNode, toFitHeight height: CGFloat){
        var aspectRatio = sprite.size.width/sprite.size.height
        sprite.size.height = height
        sprite.size.width = height * aspectRatio
    }
    static func resizeLabel(_ label: SKLabelNode, ToFitHeight height: CGFloat, andWidth width: CGFloat) -> SKLabelNode {
